...
const SchedulePage = () => {
  ...
  const fetchSchedules = async () => {
    const requestSchedules = await scheduleService.getSchedules(auth.customer.active_unit)
              ...
              {schedulesThisYear.map((item) => (
                <View p="4" rounded="2xl" bg="gray.900" key={item.id}>
                  <View flexDir="row" justifyContent="space-between" mb="2">
                    <Text color="white" fontSize="md" fontWeight="bold">#{item.session} </Text>
                    <Text color="white" fontSize="md" fontWeight="bold">{moment(item.reservated_at).format('DD MMM YYYY, HH:mm')}</Text>
                  </View>
